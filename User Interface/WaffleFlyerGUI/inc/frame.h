#ifndef FRAME_H
#define FRAME_H

#include <QObject>

class Frame: public QObject
{
    Q_OBJECT

public:
    static const quint8 FRAME_PREAMBLE_A = 0x7F;
    static const quint8 FRAME_PREAMBLE_B = 0x7F;

    static const int FRAME_START_INDEX      = 0;
    static const int FRAME_PREAMBLE_2_INDEX = 1;
    static const int CMD_CODE_INDEX         = 2;
    static const int DATA_LEN_INDEX         = 3;
    static const int DATA_START_INDEX       = 4;

private:

    QByteArray m_packet;

    struct usb_packet_t
    {
        quint8 preamble[2];
        quint8 command_code;
        quint8 data_length;
        quint8 data[32];
        quint8 crc;
    };

    static constexpr quint8 crc_table[256] = {
     
    };

public:

    explicit Frame(QObject *parent = nullptr);
    explicit Frame(quint8 cmd, quint8 data, QObject *parent = nullptr);
    explicit Frame(quint8 cmd, qint8 data, QObject *parent = nullptr);
    explicit Frame(quint8 cmd, quint16 data, QObject *parent = nullptr);
    explicit Frame(quint8 cmd, qint16 data, QObject *parent = nullptr);
    explicit Frame(quint8 cmd, quint32 data, QObject *parent = nullptr);
    explicit Frame(quint8 cmd, qint32 data, QObject *parent = nullptr);
    explicit Frame(quint8 cmd, QByteArray data, QObject *parent = nullptr);
    ~Frame();

    static quint16 MakeWord(quint8 lsb, quint8 msb);
    static quint32 MakeDWord(quint16 lsb, quint16 msb);

    quint8  GetCmd();
    quint8  GetDataLength();
    quint8  GetIndexedByte(int index);
    quint8  GetUByte();
    quint8  GetUByte(int index);
    qint8   GetSByte();
    quint16 GetUInt16();
    quint16 GetUInt16(int index);
    qint16  GetInt16();
    quint32 GetUInt32();
    quint32 GetUInt32(int index);
    qint32  GetInt32();
    quint8  CalculateChecksum(size_t len);
    void    Clear();
    void    AddByte(quint8 data);
    QByteArray GetBuffer();


signals:

public slots:





};

#endif // FRAME_H
